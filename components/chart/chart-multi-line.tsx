...
export function ChartMultiLine({
  ...
  const seriesData = useMemo(() => {
    ...
    return result;
  }, [data, colors, effectiveVisibleSeries, flowPriceUsd]);
  ...
  const initializeChart = useCallback(() => {
    ...
    try {
      ...
      if (!tooltipRef.current) {
        ...
        tooltip.className =
          "absolute text-white shadow-xl px-3 py-2.5 text-xs rounded-xl pointer-events-none hidden z-10 backdrop-blur-md border";
        tooltip.style.background = "rgba(23, 23, 23, 0.96)";
        tooltip.style.borderColor = "rgba(82, 82, 82, 0.5)";
        tooltip.style.color = "rgb(250, 250, 250)";
        ...
        if (hasData) {
              ...
              (item) => `
                <div class="flex items-center justify-between gap-4 py-1">
                  <div class="flex items-center gap-2">
                    ...
                    <span class="text-neutral-400 text-xs font-medium">${item.name}</span>
                  </div>
                  <span class="text-neutral-100 font-semibold text-xs font-mono">${formatValue(item.value, 4, isUsd)}</span>
                </div>
              `,
            )
            .join("");
          ...
          tooltip.innerHTML = `
            <div class="flex flex-col gap-2.5 min-w-44">
              <div class="text-[10px] text-neutral-500 border-b border-neutral-700/50 pb-2">
  ...
  if (!data || Object.keys(data).length === 0) {
    return (
      <div className={`relative ${className || ""}`}>
        <div
          className="w-full border-t border-b border-neutral-800 flex items-center justify-center"
          style={{ height }}
        >
  ...
  return (
    <div className={`relative z-0 ${className || ""}`}>
          ...
          {seriesData.map((series) => (
            <button
              ...
              className={`
                ...
                hover:bg-neutral-800 hover:border-neutral-700 hover:text-neutral-100
              `}
      ...
      <div
        ...
        className="w-full border-t border-b border-neutral-800 relative z-0"
        style={{ height, minHeight: height }}
      />

      {!isChartReady && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-sm text-neutral-400 flex items-center gap-2.5">
